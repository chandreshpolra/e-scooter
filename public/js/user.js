...
function changePage(page) {
  ...
  // Show loading state
  blogGrid.innerHTML = '<div class="text-center p-4"><div class="spinner-border" role="status"><span class="visually-hidden">Loading...</span></div></div>';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  initFAQ();// Blog pagination functionality
  ...
  function changePage(page) {
    ...
    fetch(`/api/blogs?page=${page}`)
      ...
      .then(data => {
        if (data.blogs && data.blogs.length > 0) {
          renderBlogs(data.blogs); // ✅ Must exist
                  ...
                  <div class="blog-card">
                    ...
                    <div class="blog-content">
                      ...
                      <p class="blog-subtitle">
                        There was an error loading the blogs. Please refresh the page.
                      </p>
                    </div>
                  </div>`;
  ...
  function renderBlogs(blogs) {
    ...
    let blogsHTML = blogs.map(blog => {
        // ✅ Date handling same as your EJS logic
        let dateToUse = null;
        if (blog.publishedDate && blog.publishedDate !== "undefined" && blog.publishedDate !== "null") {
            dateToUse = blog.publishedDate;
        } else if (blog.createdAt) {
            dateToUse = blog.createdAt;
        }
